...
const ProductForm = () => {
  ...
  // Existing media previews (from DB) when editing
  const [existingImagePreviews, setExistingImagePreviews] = useState([]);
  const [existingVideoPreviews, setExistingVideoPreviews] = useState([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Prepare existing media previews from DB
        const imgs = Array.isArray(p.media)
          ? p.media.filter((m) => m.type === 'image').sort((a, b) => (a.order || 0) - (b.order || 0))
          : [];
        const vids = Array.isArray(p.media)
          ? p.media.filter((m) => m.type === 'video').sort((a, b) => (a.order || 0) - (b.order || 0))
          : [];
  ...
  useEffect(() => {
    if (!imageFiles || imageFiles.length === 0) return; // keep existing previews when no new files
  ...
  useEffect(() => {
    if (!videoFiles || videoFiles.length === 0) return; // keep existing previews when no new files
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-3">
                    ...
                    {existingImagePreviews.length > 0 && imagePreviews.length === 0 && (
                  ...
                  <div className="md:col-span-3">
                    ...
                    {existingVideoPreviews.length > 0 && videoPreviews.length === 0 && (
